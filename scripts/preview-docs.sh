...
# Check Java version (simple check)
"$JAVA_CMD" -version
...
# Create /ontology redirect for local testing (optional, but good for consistency)
mkdir -p "${OUTPUT_FOLDER}/ontology"
cp "${OUTPUT_FOLDER}/index.html" "${OUTPUT_FOLDER}/ontology/index.html"

# Serve
echo "--------------------------------------------------------"
echo "Documentation generated in ${OUTPUT_FOLDER}/"

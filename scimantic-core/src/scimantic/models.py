"""
Scimantic models for scientific research entities.

Implements Evidence, Hypothesis, Design, and other research artifacts
as Python classes that serialize to RDF following the Scimantic ontology
(scimantic-core/ontology/scimantic.ttl) built on W3C PROV-O.
"""
...
from scimantic.config import PROV_ONTOLOGY_URI, SCIMANTIC_ONTOLOGY_URI
...
class Evidence:
    ...
    Serializes to RDF as both scimantic:Evidence and prov:Entity to enable
    provenance tracking through the research workflow.

    Schema: scimantic-core/ontology/scimantic.ttl
    """
    ...
    def to_rdf(self) -> Graph:
        """
        Serialize Evidence to RDF graph following PROV-O and Scimantic ontology.

        Returns RDF graph with:
        - Dual typing: scimantic:Evidence and prov:Entity
        - scimantic:content for the extracted finding
        ...
        # Dual typing: scimantic:Evidence AND prov:Entity
        g.add((evidence_uri, RDF.type, SCIMANTIC.Evidence))
        g.add((evidence_uri, RDF.type, PROV.Entity))

        # Content (Scimantic-specific property)
        g.add((evidence_uri, SCIMANTIC.content, Literal(self.content)))
